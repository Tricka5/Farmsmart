...
class UpdatePasswordPage extends StatefulWidget {
  ...
  const UpdatePasswordPage({super.key, required this.email, required this.userId});
...
class _UpdatePasswordPageState extends State<UpdatePasswordPage> {
  ...
  Future<void> _updatePassword() async {
    ...
    // Password strength check: minimum 8 characters, must contain letters and numbers
    ...
    try {
      final response = await http.put(
        Uri.parse('http://192.168.1.123:3000/user-aunthentication/update-password'),  // Update with the actual URL
      ...
      if (response.statusCode == 200) {
        setState(() {
          ...
          // Navigating to the Home Screen with user ID and email
          Navigator.pushReplacement(
            context,
            MaterialPageRoute(
              builder: (context) => FarmSmartScreen(
                currentUserId: widget.userId, // Pass the userId from the constructor
                currentUserEmail: widget.email, // Pass the email from the constructor
              ),
            ),
          );
        });
