...
class UserProfilePage extends StatefulWidget {
  ...
  const UserProfilePage({super.key, required this.userId, required this.myUserId});
...
class _UserProfilePageState extends State<UserProfilePage> {
  ...
  Future<Map<String, dynamic>> _fetchCommonInboxData() async {
    try {
      final response = await http.get(
        Uri.parse('http://localhost:3000/inboxparticipants/currentinbox/${widget.userId}/${widget.myUserId}'),
      );
  ...
  Future<List<dynamic>> _fetchMessages() async {
    try {
      final response = await http.get(Uri.parse('http://localhost:3000/message/$inboxId/message'));
